fitInRangeParallel <- function(optimSIRMulti, i, offsetTimes, offsetData, initConds, initParams, epiTypes, ts, k, plotConfig, p) {
	...
	# # SSE plot
	# points <- 100
	# startTimes <- seq(-5,5,length=points)
	...
	# 	sseVal[i] <- sseMulti(c(-9.230255, -2.9619520, 8.306311, 1.705483, -9.147487, -2.255504, 8.291407, startTimes[i]), offsetTimes, offsetData, initConds, epiTypes, ts, k, fineTimeStep)
	...
	# }
	# setEPS()
	# graphName <- paste("SSEFineLineDecLower", ".eps", sep='')
	# postscript(paste(plotConfig$fileName, graphName, sep=''))
	# plot(startTimes, sseVal, xlab="Start Time", ylab="SSE", type="l")
	# title(main="Change in SSE due to Start Time (Fine Time Evaluation with Reduced Lower Limit)", cex.main=1, cex.axis=0.8)
	# # readline()
	# # plot(s0Vals, sseS0)
	# # setEPS()
