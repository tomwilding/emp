evalSIRMulti <- function(times, data, initConds, params, ts, k) {
	require(deSolve)
	# source('sir.r')
	...
	for (i in 1:k) {
		...
		if (i < k) {
			t1Index <- which(times == times[ts[i+1]])
