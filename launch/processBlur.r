...
initParams <- c(logit(1e-6, 0.001, 1), logit(1e-4, 0.01, 1), log(1000), 0, 
				logit(1e-6, 0.001, 1), logit(1e-4, 0.01, 1), log(1000), 0,
				log(0.01),
				logit(1e-6, 0.001, 1), logit(1e-4, 0.01, 1), log(1000), 0)
				# log(0.01))
# Epidemic type array epidemic types correspond to the number of parameters of the sub epidemic model
epiTypes <- c(0)
epiTypes <- c(0, 4, 4, 1, 4)#, 1)
# epiTypes <- c(0, 4, 4, 1)
# Init Conds = S0, I0, R0
...
plotConfig <- list(title="Synthedemic Decomposition of Robin Thicke BitTorrent Downloads", fileName="output/graphs/blurOT/", dataFile="output/data/blurt1/blurData3.RData", envFile="output/data/blur2/blurEnv.RData", pat=12, rat=60)
